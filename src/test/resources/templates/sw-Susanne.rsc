...
/interface ethernet
	set [ find mac-address=B2:8E:06:FA:70:90 ] master-port=none speed=1Gbps name=customer-1
...
# configured IPs
	add address=172.17.3.1/24 interface=customer-1
...
/ip pool
	remove numbers=[find]
	add name=customer-1_pool ranges=172.17.3.20-172.17.3.100
...
/ip firewall filter
	...
	add action=reject chain=forward in-interface=customer-1 reject-with=icmp-admin-prohibited src-address=!172.17.3.0/24
	add action=reject chain=input in-interface=customer-1 reject-with=icmp-admin-prohibited src-address=!172.17.3.0/24
/ip firewall nat
	remove numbers=[find]
